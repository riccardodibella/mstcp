...
if [[ "$1" == "--local" ]]; then
    ...
    if [[ "$1" == "--ms_test" ]]; then
    gcc main.c -O3 -g -DMAIN_MODE=CLIENT -DMS_ENABLED=true -DNO_DEBUG -o client_ms.out
    gcc main.c -O3 -g -DMAIN_MODE=CLIENT -DMS_ENABLED=false -DNO_DEBUG -o client_tcp.out
    else
    gcc main.c -O3 -g -DMAIN_MODE=CLIENT -o client.out
    fi
fi
...
if [[ "$1" == "--local" ]]; then
    ...
    ssh dibella@172.104.237.69 'gcc main.c -O3 -g -DMAIN_MODE=SERVER -o np'
fi
