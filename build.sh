...
if [[ "$1" == "--local" ]]; then
    gcc 14.c -O3 -fno-inline-functions -g -DMAIN_MODE=CLIENT -DLOCAL_SERVER -o client.out
else
...
if [[ "$1" == "--local" ]]; then
    gcc 14.c -O3 -fno-inline-functions -g -DMAIN_MODE=SERVER -DLOCAL_SERVER -o server.out
else
