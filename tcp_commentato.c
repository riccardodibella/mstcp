...
/*
	...
	Length: 4 Bytes
	Value: 1424 Bytes (0x0590)
*/
unsigned char  mssopt[4] = { 0x02, 0x04, 0x05, 0x90 }; // This option decides the maximum size of the payload of incoming segments
...
/*
	returns the number of microseconds elapsed since the last reset of the clock.
	If cmd == 1 we reset the clock (and we return 0).
	This function is called with cmd == 1 (resetting the clock) only inside
	myconnect and myaccept
*/
...
/*
	checksum used for TCP segments
	called only inside update_tcp_header 
	parameters: 
	b1 = pointer to pseudo header
	len1 = length of the pseudo header
	b2 = pointer to segment header
	len2 = length of segment header
*/
...
/* 
	checksum used for IP and ICMP packets 
	called only inside forge_ip
*/
unsigned short int checksum ( char * b, int len){
	/*
		This whole function is the same of doing
		return (0xFFFF-compl1(b, len));
	*/
	unsigned short total = 0;
	unsigned short prev = 0;
	unsigned short *p = (unsigned short * ) b;
...
/* send_ip is called only inside mytimer, and its return value is ignored */
int send_ip(unsigned char * payload, unsigned char * targetip, int payloadlen, unsigned char proto){ /*R 210*/
	static int losscounter;
	int i, t, len;
	unsigned char destmac[6];
	/* 
		The size of this should be 1500 B (Ethernet MTU), but the TCP Maximum Segment Size is 1400 B so 
		we always stay within the 1500 B limit
	*/
	...
	/* For some reason, we always lose the 26th TX */
	...
	if( ((*(unsigned int*)targetip) & (*(unsigned int*) mask)) == ((*(unsigned int*)myip) & (*(unsigned int*) mask)))
		t = resolve_mac(*(unsigned int *)targetip, destmac); // Destination is in the local network
	else
		t = resolve_mac(*(unsigned int *)gateway, destmac); // Packet sent to the gateway towards another network
	if(t==-1) 
		return -1;
...
#define TCP_PROTO 6 // ULP inside the IP packet (vi /etc/protocols)
#define MAX_FD 8 // We can have at most 8 sockets active in our system
/*
	MSS = Maximum Segment Size
	Normally, the MTU (Maximum Transfer Unit) is 1500 B and the two headers (IP and TCP) are 20 B each
	so the MSS is usually 1460 B. In this case, we fix the MSS to 1400 B to have a safety margin.
	NOTE: if you change this to be around 2000 or more, you MUST change maximum packet length in send_ip.
*/
#define TCP_MSS 1400
...
struct txcontrolbuf{
	...
	int totlen; // TCP header + TCP options + TCP payload (application data)
	int payloadlen; // Only application data
	long long int txtime;
	struct txcontrolbuf * next;
	int retry;
};

struct tcpctrlblk{ /*R 343*/
	...
	/*
	Counts the number of payload bytes correctly received (without holes) since the start of the connection.
	This number includes a +1 in case the FIN from the remote peer has been received in-order and the
	transmission from the remote peer has ended.
	This is added to ack_offs in update_tcp_header to obtain the ack number for every outgoing segment
	*/
	unsigned int cumulativeack;
	unsigned int ack_offs, seq_offs;
	long long timeout; // in number of ticks
	/*
	Counts the number of payload bytes that we want to transmit on the connection.
	The count happens inside prepare_tcp, adding to this counter the payload size of every segment
	that we insert in the transmission queue.
	The sequence number at the beginning of each packet is seq_offs+sequence (where sequence does
	not yet include the payload of the current packet).
	This number does not consider the FIN at the end of the connection.
	*/
	unsigned int sequence;

	/*
	Decremented in mywrite for each packet added to the TX queue
	Incremented in myio when we traverse the TX queue to remove ACKed packets
	*/
	unsigned int txfree;

	unsigned int mss; // Maximum number of bytes that we transmit in the payload of a segment
	unsigned int stream_end; // Set in myio when we receive a FIN, before this it is 0xFFFFFFFF
	unsigned int fsm_timer;
	
	/* CONG CTRL*/
	#ifdef CONGCTRL
	/* 
	Values of windows/thresholds are in number of 
	segments multiplied by tcb->mss 
	*/
	...
	unsigned int cgwin;
	/*
	Accounts for the possible extra segments in flight allowed by Limited Transmit
	Note: this value is not reset after we receive a new ACK
	*/
	unsigned int lta;
	#endif
};
...
/*
	Struct associated with each socket file descriptor.
*/
struct socket_info{
...
#ifdef CONGCTRL
/* 
This function is called when a packet that we transmitted and that is still in our transmission queue is ACKed (event PKT_RCV) and when
a packet in our transmission queue is retransmitted (event TIMEOUT) TODO dire che non viene chiamato per fast transmit
*/
void congctrl_fsm(struct tcpctrlblk * tcb, int event, struct tcp_segment * tcp,int streamsegmentsize){ /*R 397*/
				...
				*/
				...
				if((tcb->repeated_acks == 1 ) || ( tcb->repeated_acks == 2)){
					if (tcb->flightsize<=tcb->cgwin + 2* (tcb->mss))
						...
						/*
						When a TCP sender has previously unsent data queued for transmission it SHOULD use the Limited Transmit algorithm, which 
						calls for a TCP sender to transmit new data upon the arrival of the first two consecutive duplicate ACKs when the following 
						conditions are satisfied:
						- The receiver's advertised window allows the transmission of the segment.
						- The amount of outstanding data would remain less than or equal to the congestion window plus 2 segments.  In other words, the
						  sender can only send two segments beyond the congestion window (cwnd).
						*/
				}
				/*
	...
	} // end if(event == PKT_RCV)
	else if (event == TIMEOUT) {
		/* 
		When we have a timeout (without fast transmit, in that case we don't call this fsm), we always restart from Slow Start 
		with a congestion window of 1 MSS and the ssthreshold adjusted depending on the previous CONGCTRL state
		*/
...
/*
We do this estimate in myio, if an ack arrives and the corresponding packet (not an earlier one, the specific corresponding packet) and all the
previously transmitted ones in the window have not been retrasmitted.
When we do this estimate, we update the value of tcb->timeout
*/
void rtt_estimate(struct tcpctrlblk * tcb, struct txcontrolbuf * node ){
	if(node->retry==1){ // If not retransmitted
		...
		tcb->timeout = MIN(MAX(tcb->rtt_e + KRTO*tcb->Drtt_e,300*1000/TIMER_USECS),MAXRTO); // expressed in number of ticks
...
int prepare_tcp(int s, unsigned char flags, unsigned char * payload, int payloadlen,unsigned char * options, int optlen){ /*R 517*/
	...
	// Add the new segment at the end of the TX queue
	txcb->next=NULL;
	if(t->txfirst == NULL){
		t->txlast = t->txfirst = txcb;
	}
	else{
		t->txlast->next = txcb; 
		t->txlast = t->txlast->next; 
	}
...
int resolve_mac(unsigned int destip, unsigned char * destmac) {
	int len,n,i;
	clock_t start;
	/* We use a buffer of the same size of the Ethernet MTU (1500 Byte) */
	unsigned char pkt[1500];
	struct ethernet_frame *eth;
	struct arp_packet *arp;
	/*
		Check the whole arpcache to see if there is an entry with
		the destination IP as key
	*/
	for(i=0;i<MAX_ARP && (arpcache[i].key!=0);i++)
		if(!memcmp(&arpcache[i].key,&destip,4)) 
			break;

	/* The condition should be (i<MAX_ARP && arpcache[i].key) */
	if(arpcache[i].key){ //If found return 
		memcpy(destmac,arpcache[i].mac,6); 
		return 0; 
	}
	// The destination IP is not already in the cache
...
/*
TCP checksum verification from an IP packet
char tcp_checksum_ok(struct ip_datagram* ip){
	struct tcp_segment* tcp = (struct tcp_segment*) ip->payload;
	uint16_t received_checksum = tcp->checksum;
	tcp->checksum = 0;
	struct pseudoheader pseudo;
	pseudo.s_addr = ip->srcaddr;
	pseudo.d_addr = ip->dstaddr;
	pseudo.zero = 0;
	pseudo.prot = 6;
	pseudo.len = htons(ip->totlen)-20;
	tcp->checksum = htons(checksum2((unsigned char*)&pseudo, 12, (unsigned char*) tcp, htons(ip->totlen)-20));
}
*/
void update_tcp_header(int s, struct txcontrolbuf *txctrl){ /*R 605*/
...
int fsm(int s, int event, struct ip_datagram * ip){ /*R 673*/
	struct tcpctrlblk * tcb = fdinfo[s].tcb;
	...
	switch(tcb->st){
		case TCP_CLOSED:
			if(event == APP_ACTIVE_OPEN) {
				...
				// In the SYN packet we send the option to tell the other peer which is the maximum segment size that we can receive
		...
		case SYN_SENT:
			if(event == PKT_RCV){
				if((tcp->flags&SYN) && (tcp->flags&ACK) && (htonl(tcp->ack)==tcb->seq_offs + 1)){
					...
					prepare_tcp(s,ACK,NULL,0,NULL,0); // Note: this ACK is never transmitted
					tcb->st = ESTABLISHED;
				}
			}
			break;
		...
		case SYN_RECEIVED:
			if(((event == PKT_RCV) && ((tcp->flags)&ACK)) &&!((tcp->flags)&SYN)){
				if(htonl(tcp->ack) == tcb->seq_offs + 1){
					...
					/* 
					Here the condition of the if should be 
					fdinfo[s].tcblist[i].st!=FREE && i<fdinfo[s].bl
					*/
...
int myconnect(int s, struct sockaddr * addr, int addrlen){ /*R 858*/
	if((addr->sa_family == AF_INET)){
		struct sockaddr_in * a = (struct sockaddr_in*) addr;
		...
		if ( s >= 3 && s<MAX_FD){
			...
			while(sleep(10)){
				if(fdinfo[s].tcb->st == ESTABLISHED ) 
					return 0;
				if(fdinfo[s].tcb->st == TCP_CLOSED ){ 
...
int mywrite(int s, unsigned char * buffer, int maxlen){ /*R 890*/
	int len,totlen=0,j,actual_len;
	if(fdinfo[s].st != TCB_CREATED || fdinfo[s].tcb->st != ESTABLISHED ){ 
		myerrno = EINVAL; 
		return -1; 
	}
	if(maxlen == 0) 
...
int myread(int s, unsigned char *buffer, int maxlen){ /*R 912*/
	int j,actual_len;
	if((fdinfo[s].st != TCB_CREATED) || (fdinfo[s].tcb->st < ESTABLISHED )){ 
		myerrno = EINVAL; 
		return -1; 
	...
	if(fdinfo[s].tcb->cumulativeack > fdinfo[s].tcb->stream_end){
		actual_len --;
	}
	if(actual_len==0){
		while(pause()){
			actual_len = MIN(maxlen,fdinfo[s].tcb->cumulativeack - fdinfo[s].tcb->rx_win_start);
			if(actual_len>0 && (fdinfo[s].tcb->cumulativeack > fdinfo[s].tcb->stream_end))
				actual_len --;
			if(actual_len!=0) 
				break;		
			if(fdinfo[s].tcb->rx_win_start){
				if(fdinfo[s].tcb->rx_win_start==fdinfo[s].tcb->stream_end){
					return 0;
				}
			}
			if((fdinfo[s].tcb->st == CLOSE_WAIT) && (fdinfo[s].tcb->unack == NULL )){
				return 0;
			} // FIN received and acknowledged
		}
	}
...
int myclose(int s){
	if((fdinfo[s].st == TCP_CLOSED) || (fdinfo[s].st == TCP_UNBOUND)) { 
		myerrno = EBADF; 
		return -1;
	}
...
void mytimer(int number){ /*R 942*/
	// "tot" is used only for CONGCTRL
	int i,tot,isfasttransmit, karn_invalidate=0;
	struct txcontrolbuf * txcb;
	if(-1 == sigprocmask(SIG_BLOCK, &mymask, NULL)){
		perror("sigprocmask"); return ;
	}
	fl++;
	tick++;
	...
	for(i=0;i<MAX_FD;i++){
		if(fdinfo[i].st == TCB_CREATED){
			...
			#ifdef CONGCTRL
			// "tot" is used to accumulate the number of bytes in the transmission queue as we traverse it
			...
			for(tot=0,txcb=tcb->txfirst;  txcb!=NULL && (tot<(tcb->cgwin+tcb->lta)); tot+=txcb->totlen, txcb = txcb->next){
				/* 
				We iterate over all the packets in the TX queue that can be in flight according to congestion control.
				This means that (if there are enough bytes in the transmission queue) we exit this loop when we the number of 
				bytes (accumulated with "tot") exceeds the current congestion window (tcb->cgwin) + the extra un-acked segments 
				allowed before we receive the third dupack for Limited Transmit (tcb->lta)
				*/
				if(txcb->retry==0) //first transmission
					fdinfo[i].tcb->flightsize+=txcb->payloadlen;
				else // This else affects the if after the #endif
			#else
			for(tot=0,txcb=tcb->txfirst;  txcb!=NULL  /*&& (tot<tcb->radwin)*/;  txcb = txcb->next){
				// If there is no CONGCTRL we traverse the whole TX queue
			#endif
				// if we have CONGCTRL
				if (karn_invalidate) 
					txcb->retry++; //a previous segment has been retransmitted, so this one cannot be used for RTO
				if(txcb->txtime+tcb->timeout > tick )  
					continue; //No timeout
				isfasttransmit = (txcb->txtime == 0); //FAST TRANSMIT for duplicate acks 
				txcb->txtime=tick;
				if(!karn_invalidate) 
					txcb->retry ++; //increment only if not already incremented by invalidation
				...
				printf("%.7ld: TX SOCK: %d SEQ:%d:%d ACK:%d Timeout = %lld FLAGS:0x%.2X (%d times)\n",
					rtclock(0),i,htonl(txcb->segment->seq) - fdinfo[i].tcb->seq_offs,
					htonl(txcb->segment->seq) - fdinfo[i].tcb->seq_offs+txcb->payloadlen,
					htonl(txcb->segment->ack) - fdinfo[i].tcb->ack_offs,
					tcb->timeout*TIMER_USECS/1000,txcb->segment->flags,txcb->retry
				);
				#ifdef CONGCTRL
				if((txcb->retry > 1) && (tcb->st >= ESTABLISHED) && !isfasttransmit)
					// Call the congestion FSM for RETXed segments
		...
		while ( 0 <= (size = recvfrom(unique_s,eth,MAXFRAME,0, (struct sockaddr *) &sll,&len))){
			...
			if (eth->type == htons (0x0806)) {
				...
				if (ip->proto == TCP_PROTO){
					...
					for(i=0;i<MAX_FD;i++)
						if((fdinfo[i].st == TCB_CREATED) && (fdinfo[i].l_port == tcp->d_port) && (tcp->s_port == fdinfo[i].tcb->r_port) && (ip->srcaddr == fdinfo[i].tcb->r_addr))
							break;
					...
					if(i<MAX_FD){
						...
						printf("%.7ld: RX SOCK:%d ACK %d SEQ:%d SIZE:%d FLAGS:0x%.2X\n", 
							rtclock(0), i, htonl(tcp->ack)-tcb->seq_offs,htonl(tcp->seq)-tcb->ack_offs,  
							htons(ip->totlen) - (ip->ver_ihl&0xF)*4 - (tcp->d_offs_res>>4)*4, tcp->flags
						);
						if(!(rand()%INV_LOSS_RATE) && g_argv[4][0]=='C') {
							printf("========== RX LOST ===============\n");
							break;
						}
						fsm(i,PKT_RCV,ip);
						...
						// After the call to the FSM the connection is still not ESTABLISHED
						if(tcb->st < ESTABLISHED)
							break; 
						...
						// streamsegment points to the payload of the segment
						...
						if(tcb->txfirst !=NULL){
							...
							if((htonl(tcp->ack)-shifter >= 0) && (htonl(tcp->ack)-shifter-(tcb->stream_end)?1:0 <= htonl(tcb->txlast->segment->seq) + tcb->txlast->payloadlen - shifter)){ // -1 is to compensate the FINù
								// If we enter here we know that this packet is ACKing some packet that we have in the transmission queue
								while((tcb->txfirst!=NULL) && ((htonl(tcp->ack)-shifter) >= (htonl(tcb->txfirst->segment->seq)-shifter + tcb->txfirst->payloadlen))){ //Ack>=Seq+payloadlen
									// Traverse the transmission queue to remove the packets that the received packet is acking
									struct txcontrolbuf * temp = tcb->txfirst; // "temp" is the first packet in the TX queue now, and it has been acked by our RXed packet
									tcb->txfirst = tcb->txfirst->next; // remove "temp" from the TX queue
									;//printf("Removing seq %d\n",htonl(temp->segment->seq)-tcb->seq_offs);
									fdinfo[i].tcb->txfree+=temp->payloadlen; // Update the free space (used for mywrite)

									#ifdef CONGCTRL
									if(htonl(tcp->ack)-shifter ==(htonl(temp->segment->seq)-shifter + temp->payloadlen)) // Exact ACK matching: estimates
										// If the ACK is referring to this specific packet
									...
									fdinfo[i].tcb->flightsize-=temp->payloadlen; // Remove the size of the ACKed payload from the flightsize
									#endif
								...
								#ifdef CONGCTRL
								// For each ACKed segment we call congctrl_fsm
								congctrl_fsm(tcb,PKT_RCV,tcp,streamsegmentsize);
								#endif

								/*
								The advertised window of the other peer is the value of window of this packet.
								Note that at this point we are sure that this packet has provided some new ACK information
								and it has acked a packet that we have txed and that was not acked before
								*/
						...
						if(((stream_offs + streamsegmentsize - tcb->rx_win_start)<RXBUFSIZE)){
							// The packet is inside the window of our receive buffer
...
int mylisten(int s, int bl){ /*R 1144*/
	if (fdinfo[s].st!=TCP_BOUND) {
		myerrno=EBADF; 
		return -1;
	}
...
int myaccept(int s, struct sockaddr * addr, int * len){ /*R 1156*/
	int i,j;
	if (addr->sa_family == AF_INET){
		struct sockaddr_in * a = (struct sockaddr_in *) addr;
		*len = sizeof(struct sockaddr_in);
		if (fdinfo[s].tcb->st!=LISTEN) {
			myerrno=EBADF; 
			return -1;
		}
		if (fdinfo[s].tcblist == NULL) {
			myerrno=EBADF; 
			return -1;
		}
		do{
...
int main(int argc, char **argv){ /*R 1188*/
	clock_t start;
	fl = 0;
	...
	if(argc>=5 && !strcmp(argv[4],"CLN")){
		...
		if ( mywrite(s,httpreq,strlen(httpreq))==1) { 
			myperror("Mywrite Failed\n"); 
			return -1;
		}
		...
		if(argc>=5 && strcmp(argv[4],"SRV")){
			printf("Warning: unknown parameter \"%s\" -  assuming SRV...\n",argv[4]);
		}
		s =  mysocket(AF_INET, SOCK_STREAM, 0);
		if ( s == -1 ){ 
		...
		if ( mybind(s,(struct sockaddr *)&addr, sizeof(struct sockaddr_in)) == -1) {
			perror("bind fallita"); 
			return 1;
		}
		if ( mylisten(s,5) == -1 ) { 
			myperror("Listen Fallita"); 
			return 1; 
		}
		len = sizeof(struct sockaddr_in);
		while(1){
			...
			if ( s2 == -1 ) { 
				myperror("Accept Fallita"); 
				return 1;
			}
			t = myread(s2,request,4999);
			if ( t == -1 ) { 
				myperror("Read fallita"); 
				return 1;
			}
			request[t]=0;
			printf("\n=====================> %s\n",request);
	...
	} // end while <- WRONG COMMENT
	/******************* END OF SERVER CODE**************/
		   
	start = tick;
	// Closes the client program after 1s (1.000.000 usec)
	while (sleep(2)){
		if(((tick-start)*TIMER_USECS)>1000000) 
			break;
	}
	return 0;
}